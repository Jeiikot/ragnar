...
allowedTools:
  ...
  - WebSearch
  - WebFetch
skills: ddd-architecture, python-conventions, react-conventions
---
...
> Your active skills (`ddd-architecture`, `python-conventions`, `react-conventions`) contain the full architecture patterns, layer rules, and coding conventions. Use them as your authoritative reference when reviewing or advising.
...
│   │   └── indexing/service.py       # index_directory, index_zip_bytes, index_documents
│   ├── domain/                       # Domain layer (ports + entities)
│   │   ├── chat/entities.py          # ChatResponse dataclass
│   │   └── indexing/ports/           # protocols.py, bundles.py, __init__.py
│   ├── infrastructure/               # Infrastructure / adapters layer
│   │   ├── chat/engine.py            # ChatEngine (RAG + memory), build_chat_engine
│   │   ├── indexing/                 # adapters.py, chunking.py, constants.py, file_discovery.py, pdf_reader.py, storage.py, zip_utils.py
│   │   ├── providers/                # __init__.py (facade), contracts.py, selector.py, types.py, openai.py, ollama.py, huggingface.py
...
│   │   ├── components/               # ChatWindow.tsx, IndexForm.tsx, MessageBubble.tsx
...
6. **Dependency Direction Enforcement** — Ensure domain never imports from infrastructure/api; application imports only from domain and shared; infrastructure implements domain protocols; `dependencies.py` is the sole composition root.
...
5. Reference the architecture patterns in your skills when suggesting changes.
